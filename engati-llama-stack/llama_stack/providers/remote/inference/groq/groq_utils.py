...
def _convert_message(message: Message) -> ChatCompletionMessageParam:
    if message.role == "system":
        ...
        return ChatCompletionToolMessageParam(role="tool", content=message.content, tool_call_id=message.call_id)
    elif message.role == "assistant":
        print("------------------------------------------------------------")
        print("------------------------------------------------------------")
        print("------------------------------------------------------------")
        print(f"Inside convert messages for assistant before: {message}")
        message_param = ChatCompletionAssistantMessageParam(
            role="assistant", content=message.content, tool_calls=[]
        )
        print(f"Message Param constructed is: {message_param}")
        if message.tool_calls:
            tools = []
            for tool in message.tool_calls:
                print(f"tool arguments identified are: {tool.arguments} with type as: {type(tool.arguments)}")
                function = Function(name=tool.tool_name, arguments=json.dumps(tool.arguments))
                tool_call = ChatCompletionMessageToolCallParam(id=tool.call_id, type="function", function=function)
                tools.append(tool_call)
            message_param["tool_calls"] = tools
        print(f"Inside convert messages for assistant after: {message_param}")
        print("------------------------------------------------------------")
        print("------------------------------------------------------------")
        print("------------------------------------------------------------")
